...
RSpec.feature "ShopProfile", type: :feature do
  ...
  scenario 'fail to register shop profile by xem validation' do
    invalid_address = 'naughty invalid address'
    visit root_path
    click_link '店舗情報作成'
    fill_in 'shop_profile[name]', with: '店舗１'
    fill_in 'shop_profile[xem_address]', with: invalid_address
    click_button '作成'
    expect(page).to have_content '無効なアドレスです'
  end 
